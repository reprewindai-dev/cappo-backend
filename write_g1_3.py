...
def test_g1_3_wan_off_wrong_scope_fails(db: Session):
    ...
    mount_record, anchor, reason = reg.request_mount(
        ...
        role="agent", policy=MountPolicy(), ttl_seconds=3600,
    ...
    with no_wan() as blocked:
        ...
        decision, dec_reason, _anchor, binding = reg.evaluate(
            mount_id=mount_id, action=WRONG_ACTION, token_id=token_id, nonce=nonce,
    ...
    receipts = [r for r in db.query(CapabilityActionReceipt).filter_by(execution_id=exec_id).all()]
    assert len(receipts) == 1
    assert receipts[0].decision.upper() == "DENY"
    assert receipts[0].signed_receipt_cose is not None
...
def test_g1_3_wan_off_replay_fails(db: Session):
    ...
    assert receipts[0].decision.upper() == "ALLOW"
    assert receipts[1].decision.upper() == "DENY"
