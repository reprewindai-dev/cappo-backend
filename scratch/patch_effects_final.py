...
class ConsequenceContext:
    ...
    import re
    adapter_pattern = re.compile(r'class GovernedCounterAdapter\(TargetAdapter\):.*?raise ValueError\(f"unsupported_action: \{context\.action\}"\)', re.DOTALL)
    ...
    def dispatch(self, context: ConsequenceContext) -> object:
        ...
        raise ValueError(f"unsupported_action: {context.action}")'''
