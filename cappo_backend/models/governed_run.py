...
Forward-constructed from ``VeklomRun`` (migration note §1/§3) but owns the
*pre-execution* lifecycle rather than being a post-hoc projection. It carries the
proven hash columns as first-class typed JSON, the run state machine's current
``state``, the governance context produced before execution, and the minted
``execution_identity`` (EI Plan §Persistence model — new JSONB column).
"""
...
class GovernedRun(Base):
    ...
    # PGL linkage: {"pre_execution_certificate_id": ..., "post_execution_certificate_id": ...}
