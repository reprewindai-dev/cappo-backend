...
class VeklomPGLClient:
    ...
    def __init__(self, settings: Settings | None = None) -> None:
        ...
        self._base_url = self._settings.veklom_byos_backend_url or os.getenv(
            "VEKLOM_BYOS_BACKEND_URL", ""
        )
    ...
    def validate_agent_for_execution(
        ...
        # Check agent status
        if cert.is_revoked:
            raise CertificateInvalidError(f"Agent {agent_id} has been revoked. Terminated.")

        if not cert.is_active:
            raise CertificateInvalidError(f"Agent {agent_id} is not active (status: {cert.status})")
        ...
        if requested_tools:
            ...
            for tool in requested_tools:
                if tool not in allowed_tools:
                    raise CertificateInvalidError(
                        f"Agent {agent_id} not authorized for tool: {tool}. Allowed: {cert.tools}"
                    )
