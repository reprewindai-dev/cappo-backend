"""cAPI registration and liveness maintenance for CAPPO."""
...
async def register_with_capi(
    ...
    """Register CAPPO once; callers decide when a failed attempt is retried."""
    ...
    if response.status_code in (200, 201):
        logger.info("CAPPO registered with cAPI")
        return True
...
async def heartbeat_until_missing(
    ...
    """Refresh CAPPO registration until cAPI reports it missing or shutdown begins."""
    ...
    while not stop.is_set():
        ...
        try:
            async with httpx.AsyncClient(timeout=REGISTRATION_TIMEOUT_SECONDS, transport=transport) as client:
                response = await client.post(url, json={"service_name": "cappo"}, headers=_headers(settings))
        except httpx.HTTPError as exc:
        ...
        if response.status_code == 404:
            logger.info("CAPPO cAPI registration is missing; re-registering")
            return True
...
async def maintain_capi_registration(
    ...
    """Keep CAPPO registered without treating transport health as authority."""
    while not stop.is_set():
