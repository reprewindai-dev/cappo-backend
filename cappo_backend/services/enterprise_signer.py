...
class KMSSigner:
    ...
    algorithm: Literal[
        "ECDSA_SHA_256", "ECDSA_SHA_384", "ECDSA_SHA_512", "RSASSA_PKCS1_V1_5_SHA_256"
    ] = "ECDSA_SHA_256"
    ...
    def _get_client(self) -> Any:
        ...
        if self._client is None:
            try:
                ...
                raise SigningError(
                    "boto3 is required for AWS KMS signing. Install: pip install boto3"
                )
    ...
    def verify(self, payload: Any, signature: str) -> bool:
        ...
        try:
            ...
            raise SigningError(
                "cryptography library required for local verification. Install: pip install cryptography"
            )
        except Exception as e:
    ...
    def _get_clients(self) -> tuple[Any, Any]:
        ...
        if self._crypto_client is None:
            try:
                ...
                raise SigningError(
                    "azure-keyvault-keys and azure-identity required. Install: pip install azure-keyvault-keys azure-identity"
                )
            except Exception as e:
...
def create_enterprise_signer(provider: Literal["aws", "azure", "vault", "hmac"], **kwargs) -> Any:
