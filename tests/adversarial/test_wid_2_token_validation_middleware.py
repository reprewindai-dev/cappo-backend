...
def validator(replay_cache):
    return IdentityValidator(expected_audience="https://api.veklom.local", replay_cache=replay_cache)
...
def test_3_governed_missing_ect(validator):
    ...
    with pytest.raises(MissingExecutionContextError):
        ctx.enforce(route="/data", method="GET", trace_id="t1", htu="/data", body_hash="", wit_payload=make_valid_wit())
...
def test_4_state_changing_missing_wpt(validator):
    ...
    with pytest.raises(MissingWorkloadProofError):
        ctx.enforce(route="/update", method="POST", trace_id="t1", htu="/update", body_hash="",
                    wit_payload=make_valid_wit(), ect_payload=make_valid_ect())
...
def test_5_consequence_missing_authority(validator):
    ...
    with pytest.raises(MissingAuthorityError):
        ctx.enforce(route="/transfer", method="POST", trace_id="t1", htu="https://api.veklom.local/submit", body_hash="body_hash_val",
                    wit_payload=make_valid_wit(), ect_payload=make_valid_ect(), wpt_payload=make_valid_wpt())
...
def test_7_expired_ect(validator):
    ...
    ctx = WIDMiddlewareContext(RouteClassification.GOVERNED, validator)
    with pytest.raises(TokenExpiredError):
        ctx.enforce("/data", "GET", "t1", "/data", "", wit_payload=make_valid_wit(), ect_payload=ect)
...
def test_8_expired_wpt(validator):
    ...
    ctx = WIDMiddlewareContext(RouteClassification.STATE_CHANGING, validator)
    with pytest.raises(TokenExpiredError):
        ctx.enforce("/update", "POST", "t1", "https://api.veklom.local/submit", "body_hash_val",
...
def test_10_body_hash_mismatch(validator):
    ...
    with pytest.raises(BodyHashMismatchError):
        ctx.enforce("/update", "POST", "t1", "https://api.veklom.local/submit", "WRONG_HASH",
                    wit_payload=make_valid_wit(), ect_payload=make_valid_ect(), wpt_payload=wpt)
...
def test_14_authority_hash_mismatch(validator):
    auth = make_valid_authority()
    auth["_mock_hash"] = "WRONG_AUTH_HASH"  # middleware will use this instead of "auth_hash_val"
    ctx = WIDMiddlewareContext(RouteClassification.CONSEQUENCE, validator)
...
def test_18_replayed_wpt_jti(validator):
    wpt = make_valid_wpt()
    wit = make_valid_wit()
    wit["jti"] = "wit_jti_222" # avoid wit replay
    ctx = WIDMiddlewareContext(RouteClassification.STATE_CHANGING, validator)
    ctx.enforce("/update", "POST", "t1", "https://api.veklom.local/submit", "body_hash_val",
                wit_payload=wit, ect_payload=make_valid_ect(), wpt_payload=wpt)
    wit["jti"] = "wit_jti_333"
    with pytest.raises(ReplayDeniedError):
        ctx.enforce("/update", "POST", "t1", "https://api.veklom.local/submit", "body_hash_val",
                    wit_payload=wit, ect_payload=make_valid_ect(), wpt_payload=wpt)
...
def test_21_denial_emits_structured_evidence(validator):
    ...
    except MissingWorkloadIdentityError as e:
...
def test_22_validation_does_not_mutate_p5_truth_state():
    """22. validation does not mutate P5 truth state"""
    # WID Middleware has no DB connection and operates purely on payloads
    pass

def test_23_validation_does_not_mark_outbox_sent():
    """23. validation does not mark outbox SENT"""
    pass

def test_24_validation_does_not_create_fake_pgl_proof():
    """24. validation does not create fake PGL proof"""
    pass
