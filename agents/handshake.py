...
async def login(client: httpx.AsyncClient, email: str, password: str) -> str | None:
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            fail("Wrong credentials — check email/password")
            return None
        elif r.status_code == 500:
            fail("Backend 500 on login — database not connected. Check Coolify env vars.")
            return None
        else:
...
def save_token(token: str, email: str):
    ...
    lines = [
        "# Auto-generated by agents/handshake.py",
        ...
        "OLLAMA_MODEL=llama3",
        "VEKLOM_AGENT_PROVIDER=ollama",
        "AGENT_MAX_ITER=10",
        "AGENT_DEBUG=0",
    ]
    env_path.write_text("\n".join(lines) + "\n")
    ok("Saved to .env.agent — all agents will use this token automatically")
...
async def run_handshake(email: str, password: str):
    ...
    if failed == 0:
        ...
        print("    python agents/agent_ollama.py")
        print("    python agents/agent_router.py")
    elif passed >= 3:
...
if __name__ == "__main__":
    ...
    if not args.email or not args.password:
        ...
        print("Usage: python agents/handshake.py --email you@email.com --password yourpass")
        print()
        args.email = ""
        args.password = ""
