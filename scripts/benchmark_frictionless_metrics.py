...
def live_ttfgc() -> Tuple[
    ...
    try:
        payload = json.loads(
            os.environ.get("CAPPO_BENCH_ACTION_PAYLOAD", "{}")
        )
    except Exception as exc:
        return (
            None,
            None,
            f"CAPPO_BENCH_ACTION_PAYLOAD invalid JSON: {exc}",
        )
    ...
    try:
        execution_id = os.environ.get("CAPPO_BENCH_EXECUTION_ID", "exec-fric-001")
        mount = client.mount_capability(
            package_ref=capability_id,
            execution_id=execution_id,
            execution_scope={"workspace": "test-workspace", "project": "test-project"},
            requested_action_scope={"reads": [], "writes": ["execute"], "blocked": []},
            ttl_seconds=300
        )

        # Set the client's bearer token so it can dispatch
        client.bearer_token = mount.bearer
        client.session.headers.update({"Authorization": f"Bearer {mount.bearer}"})

        dispatch = client.dispatch_consequence(
            capability_lease=mount,
            action=payload.get("action", "execute"),
            resource=payload.get("resource", "provider-dispatch"),
...
def live_recovery() -> Tuple[
    Optional[int],
    Optional[Dict[str, Any]],
    Optional[str],
]:
    execution_id = os.environ.get(
        "CAPPO_BENCH_OUTCOME_UNKNOWN_EXECUTION_ID"
    )
    ...
    if not execution_id:
        return (
            None,
            None,
            "CAPPO_BENCH_OUTCOME_UNKNOWN_EXECUTION_ID not supplied",
        )
...
def secret_isolation() -> Tuple[
    ...
    if not capture_path or not markers_raw:
        return (
            None,
            None,
            "CAPPO_BENCH_WORKLOAD_CAPTURE and "
            "CAPPO_BENCH_SECRET_MARKERS are required",
        )
...
def main() -> int:
    ...
    if manifest is None:
        ...
        if isinstance(files, list):
            ...
            metrics.append(
                metric(
                    "FRIC-02",
                    "INDETERMINATE",
                    error="config_files_touched missing/not a list",
                )
            )
        ...
        if isinstance(approvals, int):
            ...
            metrics.append(
                metric(
                    "FRIC-03",
                    "INDETERMINATE",
                    error="manual_approval_steps missing/not an integer",
                )
            )
        ...
        if isinstance(components, list):
            metrics.append(
                metric(
                    "FRIC-04",
                    "PASS" if len(components) <= 1 else "FAIL",
                    len(components),
                    components,
                )
            )
        ...
        setup_calls = manifest.get(
            "api_setup_calls_before_dispatch"
        )
        ...
        if isinstance(setup_calls, int):
            metrics.append(
                metric(
                    "FRIC-05",
                    "PASS" if setup_calls <= 1 else "FAIL",
                    setup_calls,
                )
            )
        else:
    ...
    recovery_calls, recovery_evidence, recovery_error = (
        live_recovery()
    )
    ...
    if recovery_calls is None:
        ...
        metrics.append(
            metric(
                "FRIC-06",
                "PASS" if recovery_calls <= 1 else "FAIL",
                recovery_calls,
                recovery_evidence,
            )
        )
    ...
    summary = {
        ...
        "metrics_passed": sum(
            1 for m in metrics if m["status"] == "PASS"
        ),
        "metrics_failed": sum(
            1 for m in metrics if m["status"] == "FAIL"
        ),
        "metrics_indeterminate": sum(
            1
            for m in metrics
            if m["status"] == "INDETERMINATE"
        ),
        "metrics": metrics,
    }
