...
This skill enforces "verify-before-incurring" for backend routing in `cappo-backend`. It runs a local simulation to verify policy, cost, and risk before any live execution.

## When to use this skill

- When a task involves calling an external LLM or tool that could incur compute cost or leak data from `cappo-backend`.
...
1. **Gather Input Parameters**
   - Task prompt
   - Tenant's policy and budget settings (verified via PGL)
...
- Always run the simulation before any model call that might incur cost or risk in `cappo-backend`.
