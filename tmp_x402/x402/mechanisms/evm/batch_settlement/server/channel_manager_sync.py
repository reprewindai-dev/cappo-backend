"""Sync `BatchSettlementChannelManagerSync` for periodic claim/settle/refund."""
...
class BatchSettlementChannelManagerSync:
    """Sync channel manager for claim / settle / refund lifecycle."""

    def __init__(self, config: ChannelManagerConfigSync) -> None:
    ...
    def settle(self) -> SettleResult:
        ...
        response = self._facilitator.settle(payload, requirements)
    ...
    def _update_claimed_sessions(self, claims: list) -> None:
        storage = self._scheme.get_storage()
        for claim in claims:
...
__all__ = ["BatchSettlementChannelManagerSync", "ChannelManagerConfigSync"]
